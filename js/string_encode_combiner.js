...
app.registerExtension({
    name: "ComfyUI_HUD.StringEncodeCombiner",
    async nodeCreated(node) {
        const TARGET_CLASSES = ["ComfyUI_HUD_StringEncodeCombiner"];
        if (!TARGET_CLASSES.includes(node.comfyClass)) return;

        const STORAGE_KEY = "comfyui_hud_encode_sections";
        const SEPARATOR_KEY = "comfyui_hud_encode_separator";
        const SEPARATOR_PIN_KEY = "comfyui_hud_encode_separator_pinned";
        const SHOW_RESULT_KEY = "comfyui_hud_encode_show_result";
        const INCLUDE_TITLES_KEY = "comfyui_hud_encode_include_titles";
        const RUNTIME_RESULT_KEY = "comfyui_hud_encode_runtime_result";
        ...
        const styleOptions = {
            rootGradient: "linear-gradient(135deg, rgba(255,120,110,0.10), rgba(255,168,90,0.09), rgba(255,215,120,0.07), rgba(220,110,255,0.07))",
            rootBorder: "1px solid rgba(255,220,170,0.14)",
            rootBoxShadow: "0 0 0 1px rgba(255,255,255,0.03), 0 12px 30px rgba(0,0,0,0.18)",
            headerGradient: "linear-gradient(90deg, rgba(255,130,110,0.14), rgba(255,186,90,0.12), rgba(255,224,120,0.10), rgba(210,110,255,0.10))"
        };
        ...
        node.onDrawForeground = function (ctx) {
            ...
            gradient.addColorStop(0.00, "rgba(255, 100, 90, 0.13)");
            gradient.addColorStop(0.22, "rgba(255, 175, 90, 0.12)");
            gradient.addColorStop(0.44, "rgba(255, 225, 110, 0.10)");
            gradient.addColorStop(0.68, "rgba(255, 120, 185, 0.10)");
            gradient.addColorStop(0.86, "rgba(190, 120, 255, 0.11)");
            gradient.addColorStop(1.00, "rgba(120, 145, 255, 0.08)");
        ...
        let runtimeResult = String(node.properties?.[RUNTIME_RESULT_KEY] ?? "");
        ...
        function saveState() {
            ...
            node.properties[RUNTIME_RESULT_KEY] = String(runtimeResult ?? "");
        ...
        if (!document.getElementById("comfyui-hud-string-encode-combiner-style")) {
            const style = document.createElement("style");
            style.id = "comfyui-hud-string-encode-combiner-style";
            style.textContent = `
            .comfyui-hud-string-encode-combiner-node .comfyui-hud-encode-card input,
            .comfyui-hud-string-encode-combiner-node .comfyui-hud-encode-card textarea {
                outline: none; border: 1px solid rgba(255,255,255,0.14);
                background: linear-gradient(180deg, rgba(22,18,22,0.92), rgba(34,28,30,0.80));
                color: rgba(255,248,242,0.95); box-shadow: inset 0 1px 0 rgba(255,255,255,0.05);
            }
            .comfyui-hud-string-encode-combiner-node .comfyui-hud-encode-card input:focus,
            .comfyui-hud-string-encode-combiner-node .comfyui-hud-encode-card textarea:focus {
                border-color: rgba(255,220,170,0.48); box-shadow: 0 0 0 1px rgba(255,220,170,0.18);
            }`;
        ...
        container.classList.add("comfyui-hud-string-encode-combiner-node");
        ...
        const header = css(document.createElement("div"), {
            ...
            border: "1px solid rgba(255,220,170,0.14)", background: "linear-gradient(135deg, rgba(255,176,110,0.11), rgba(255,120,130,0.08), rgba(210,120,255,0.06), rgba(255,255,255,0.04))",
        ...
        const headerHint = css(document.createElement("div"), { fontSize: "11px", fontWeight: "700", lineHeight: "1.4", color: "rgba(255,250,245,0.88)" });
        headerHint.textContent = "Combine prompt blocks, forward text, and encode with CLIP in one card.";

        // --- Restored UI Functions ---
        function createToggleBtn(label, active, onClick) {
            const btn = css(document.createElement("button"), {
                padding: "6px 10px", borderRadius: "8px", border: active ? "1px solid rgba(255,220,170,0.5)" : "1px solid rgba(255,255,255,0.15)",
                background: active ? "rgba(255,220,170,0.15)" : "rgba(255,255,255,0.05)",
                color: active ? "#ffcca8" : "rgba(255,255,255,0.6)", fontSize: "11px", fontWeight: "700", cursor: "pointer",
            });
        ...
        separatorInput.oninput = () => { 
            ...
            saveState(); 
            if (showResult) resultText.textContent = composeText() || "No text content.";
        };
        ...
        const resultWrap = css(document.createElement("div"), {
            ...
            marginTop: "2px", padding: "10px 11px", borderRadius: "13px", border: "1px solid rgba(255,230,195,0.14)",
            background: "linear-gradient(180deg, rgba(36,20,18,0.84), rgba(44,30,34,0.74))",
        });
        const resultText = css(document.createElement("div"), {
            width: "100%", minHeight: "52px", padding: "11px 12px", borderRadius: "11px", border: "1px solid rgba(255,235,210,0.10)",
            color: "rgba(255,250,245,0.94)", fontSize: "11px", lineHeight: "1.6", fontFamily: "monospace", whiteSpace: "pre-wrap",
        });
        ...
        const addBtn = css(document.createElement("button"), {
            width: "100%", padding: "10px 12px", borderRadius: "12px", border: "1px dashed rgba(255,228,188,0.18)",
        ...
        function renderSections() {
            ...
            showResultBtn.style.background = showResult ? "rgba(255,220,170,0.15)" : "rgba(255,255,255,0.05)";
            showResultBtn.style.border = showResult ? "1px solid rgba(255,220,170,0.5)" : "1px solid rgba(255,255,255,0.15)";
            showResultBtn.style.color = showResult ? "#ffcca8" : "rgba(255,255,255,0.6)";

            includeTitlesBtn.style.background = includeTitles ? "rgba(255,220,170,0.15)" : "rgba(255,255,255,0.05)";
            includeTitlesBtn.style.border = includeTitles ? "1px solid rgba(255,220,170,0.5)" : "1px solid rgba(255,255,255,0.15)";
            includeTitlesBtn.style.color = includeTitles ? "#ffcca8" : "rgba(255,255,255,0.6)";
            ...
            sections.forEach((s, idx) => {
                const card = css(document.createElement("div"), {
                    ...
                    border: "1px solid rgba(255,235,210,0.12)", background: "rgba(255,255,255,0.05)",
                });
                card.classList.add("comfyui-hud-encode-card");
                ...
                const pinBtn = createActionBtn("#", "pin", s.pinned, () => { s.pinned = !s.pinned; saveState(); renderSections(); });
                ...
                const delBtn = createActionBtn("Delete", "delete", false, () => { sections.splice(idx, 1); saveState(); renderSections(); });
            ...
            if (showResult) {
                resultText.textContent = composeText() || "No text content.";
            }

            applyRootStyle();
        }

        addBtn.onclick = () => { sections.push(DEFAULT_SECTION()); saveState(); renderSections(); };
        ...
        node.addDOMWidget("comfyui_hud_string_encode_combiner", "preview", container);
        ...
        node.onConfigure = function () {
            ...
            runtimeResult = String(node.properties?.[RUNTIME_RESULT_KEY] ?? "");
            renderSections();
        };
